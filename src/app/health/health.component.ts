...
export class HealthComponent implements OnInit {
  ...
  TheSelectDisplays: FormGroup = new FormGroup({ 
    CreateNew: new FormControl('N'),
    DisplaySpecific: new FormControl('N'),
    DisplayAll: new FormControl('N'),
    CopyFile:new FormControl('N'),
    MgtCalories:new FormControl('N'),
    CalculCalories:new FormControl('N'),
    SelectedDate:new FormControl(Date()),
    theAction: new FormControl('Action'),
    ReloadHTML: new FormControl('N'),
...
SearchText(event:any){
  if (event.currentTarget.id==='search' && event.currentTarget.value!==''){
    this.checkText=event.currentTarget.value
  } else { 
    this.checkText=''; 
  }
}
...
DelAfterConfirm(event:any){
  ...
  if (  event.currentTarget.id==='YesDelConfirm'){
    if (this.theEvent.target.id.substring(0,10)==='DelAllDate'){
      ...
      if (this.theEvent.target.id.substring(0,17)==='DelAll-'){
      ...
      if (event.target.textContent.indexOf('cancel')!==-1){
        ...
        if (this.myType.trim()==="date" ){
          ...
          if (this.myAction==="insert before"){
            ...
            this.isDeleteItem=true;
            this.delMsg=
            ' ingredient ' + this.HealthAllData.tabDailyReport[this.TabOfId[0]].meal[this.TabOfId[1]].dish[this.TabOfId[1]].name
...
fillAllData(inRecord:any, outRecord:any){
    ...
    for (i=0; i<inRecord.meal.length; i++){
      ...
      for (j=0; j<inRecord.meal[i].dish.length; j++){
        ...
        outRecord.meal[iMeal].dish[ outRecord.meal[iMeal].dish.length-1].name=inRecord.meal[i].dish[j].name;
        outRecord.meal[iMeal].dish[ outRecord.meal[iMeal].dish.length-1].unit=inRecord.meal[i].dish[j].unit;
        outRecord.meal[iMeal].dish[ outRecord.meal[iMeal].dish.length-1].quantity=inRecord.meal[i].dish[j].quantity;
        }
      }
  }
